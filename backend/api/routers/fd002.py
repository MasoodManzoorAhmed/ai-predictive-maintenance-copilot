# backend/api/routers/fd002.py
"""
FD002 RUL prediction router (thin but strict).
"""

from fastapi import APIRouter, HTTPException
import pandas as pd

from backend.api.models.requests import FD002BatchRequest
from backend.api.models.responses import FD002PredictionResponse, EnginePrediction
...
@router.post("/fd002", response_model=FD002PredictionResponse)
def predict_fd002(request: FD002BatchRequest):
    try:
        ...
        if not too_short.empty:
            ...
            raise HTTPException(
                status_code=400,
                detail=(
                    f"FD002 batch prediction requires at least {seq_len} cycles per unit. "
        ...
        out = run_fd_inference("FD002", df_input=df, allow_padding=False)
        ...
        return FD002PredictionResponse(
            fd_name="FD002",
