...
DATA_PATH = Path("data/cmapps/train_FD002.txt")
...
# Use a mid window to avoid “near failure = low RUL”
START_CYCLE = 50
df_win = df_u[(df_u["cycle"] >= START_CYCLE) & (df_u["cycle"] < START_CYCLE + SEQ_LEN)].copy()

if len(df_win) < SEQ_LEN:
    # fallback to last window if mid window isn't possible
    df_win = df_u.tail(SEQ_LEN).copy()

# Batch endpoint payload (/predict/fd002)
batch_payload = {"records": df_win.to_dict(orient="records")}
Path("fd002_batch_swagger.json").write_text(json.dumps(batch_payload, indent=2))
print(" Wrote:", Path("fd002_batch_swagger.json").resolve())
...
Path("fd002_single_swagger.json").write_text(json.dumps(single_payload, indent=2))
print(" Wrote:", Path("fd002_single_swagger.json").resolve())
